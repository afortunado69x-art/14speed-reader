...
class BookParserService {
  ...
  Future<ParsedBook> _parseFb2(String path) async {
    ...
    // Strip XML tags
    // Note: replaceAllMapped used for title tags to preserve inner text
    final text = body
        .replaceAll(RegExp(r'<section[^>]*>', dotAll: true), '\n\n')
        .replaceAllMapped(RegExp(r'<title[^>]*>(.*?)</title>', dotAll: true),
            (m) => '\n\n${m.group(1) ?? ''}\n\n')
  ...
  // ─── PDF ──────────────────────────────────────────────────
  // PDF text extraction: reads raw bytes and strips binary content.
  // For production-quality extraction add the "pdfx" or "pdf_text" package.
  Future<ParsedBook> _parsePdf(String path) async {
    final name = p.basenameWithoutExtension(path);
    try {
      final bytes = await File(path).readAsBytes();
      // Quick heuristic: extract printable ASCII runs from the PDF byte stream.
      // This works for simple PDFs; complex/encrypted PDFs will yield minimal text.
      final buf = StringBuffer();
      int run = 0;
      ...
      if (raw.trim().length < 20) {
        return ParsedBook(
          title: name, author: '',
          words: ['PDF', 'loaded', '—', 'text', 'extraction', 'requires',
                  'pdfx', 'package', 'for', 'encrypted', 'or', 'scanned', 'files'],
          chapters: [Chapter(title: 'PDF Document', startWordIndex: 0)],
        );
      }
      return _buildFromText(raw, name, '');
    } catch (e) {
      return _buildFromText('Could not read PDF: \$e', name, '');
    }
  }
